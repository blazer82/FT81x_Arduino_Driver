...
void FT81x::initDisplay()
{
    ...
    DISPLAY_CMD(ST7701_BK0_PORCTRL, 20, 10); // vbp, vfp
    DISPLAY_CMD(ST7701_BK0_INVSEL, 0x00, 0x02);
    DISPLAY_CMD(0xCC, 0x10);
    DISPLAY_CMD(0xCD, 0x08);


    DISPLAY_CMD(ST7701_BK0_PVGAMCTRL, 0x02, 0x13, 0x1B, 0x0D, 0x10, 0x05, 0x08, 0x07, 0x07, 0x24, 0x04, 0x11, 0x0E, 0x2C, 0x33, 0x1D);
    DISPLAY_CMD(ST7701_BK0_NVGAMCTRL, 0x05, 0x13, 0x1B, 0x0D, 0x11, 0x05, 0x08, 0x07, 0x07, 0x24, 0x04, 0x11, 0x0E, 0x2C, 0x33, 0x1D);
    
    
    //DISPLAY_CMD(ST7701_BK0_RGBCTRL, 0x00, 50, 20);
    //DISPLAY_CMD(ST7701_BK0_PARCTRL, 0x00, 0x00, DISPLAY_HEIGHT & 0xFF, DISPLAY_HEIGHT & 0xF00);
    
    //DISPLAY_CMD(0xCC, 0x10);
    //DISPLAY_CMD(0xB0, 0x00, 0x11, 0x19, 0x0C, 0x10, 0x06, 0x07, 0x0A, 0x09, 0x22, 0x04, 0x10, 0x0E, 0x28, 0x30, 0x1C);
    //DISPLAY_CMD(0xB1, 0x00, 0x12, 0x19, 0x0D, 0x10, 0x04, 0x06, 0x07, 0x08, 0x23, 0x04, 0x12, 0x11, 0x28, 0x30, 0x1C);
    
    // Command2, BK1
    DISPLAY_CMD(ST7701_CND2BKxSEL, 0x77, 0x01, 0x00, 0x00, ST7701_CMD2BK1SEL);
    DISPLAY_CMD(ST7701_BK1_VRHS, 0x5D);
    DISPLAY_CMD(ST7701_BK1_VCOM, 0x43);
    DISPLAY_CMD(ST7701_BK1_VGHSS, 0x81);
    DISPLAY_CMD(ST7701_BK1_TESTCMD, 0x80);
    DISPLAY_CMD(ST7701_BK1_VGLS, 0x43);
    DISPLAY_CMD(ST7701_BK1_PWCTLR1, 0x85);
    DISPLAY_CMD(ST7701_BK1_PWCTLR2, 0x20);
    //DISPLAY_CMD(0xB9, 0x10);
    DISPLAY_CMD(ST7701_BK1_SPD1, 0x78);
    DISPLAY_CMD(ST7701_BK1_SPD2, 0x78);
    DISPLAY_CMD(0xD0, 0x88);
    DISPLAY_CMD(0xE0, 0x00, 0x00, 0x02);
    DISPLAY_CMD(0xE1, 0x03, 0xA0, 0x00, 0x00, 0x04, 0xA0, 0x00, 0x00, 0x00, 0x20, 0x20);
    DISPLAY_CMD(0xE2, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
    DISPLAY_CMD(0xE3, 0x00, 0x00, 0x11, 0x00);
    DISPLAY_CMD(0xE4, 0x22, 0x00);
    DISPLAY_CMD(0xE5, 0x05, 0xEC, 0xA0, 0xA0, 0x07, 0xEE, 0xA0, 0xA0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
    DISPLAY_CMD(0xE6, 0x00, 0x00, 0x11, 0x00);
    DISPLAY_CMD(0xE7, 0x22, 0x00);
    DISPLAY_CMD(0xE8, 0x06, 0xED, 0xA0, 0xA0, 0x08, 0xEF, 0xA0, 0xA0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
    DISPLAY_CMD(0xEB, 0x00, 0x00, 0x40, 0x40, 0x00, 0x00, 0x00);
    DISPLAY_CMD(0xED, 0xFF, 0xFF, 0xFF, 0xBA, 0x0A, 0xBF, 0x45, 0xFF, 0xFF, 0x54, 0xFB, 0xA0, 0xAB, 0xFF, 0xFF, 0xFF);
    ...
    // Copied from https://blog.csdn.net/arunboy/java/article/details/103714996
    /*DISPLAY_CMD(0xE0, 0x00, 0x00, 0x02);
    DISPLAY_CMD(0xE1, 0x04, 0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x00, 0x20, 0x20);
    DISPLAY_CMD(0xE2, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
    DISPLAY_CMD(0xE3, 0x00, 0x00, 0x33, 0x00);
    DISPLAY_CMD(0xE4, 0x22, 0x00);
    DISPLAY_CMD(0xE5, 0x04, 0x34, 0xAA, 0xAA, 0x06, 0x34, 0xAA, 0xAA, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
    DISPLAY_CMD(0xE6, 0x00, 0x00, 0x33, 0x00);
    DISPLAY_CMD(0xE7, 0x22, 0x00);
    DISPLAY_CMD(0xE8, 0x05, 0x34, 0xAA, 0xAA, 0x07, 0x34, 0xAA, 0xAA, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
    DISPLAY_CMD(0xEB, 0x02, 0x00, 0x40, 0x40, 0x00, 0x00, 0x00);
    DISPLAY_CMD(0xEC, 0x00, 0x00);
    DISPLAY_CMD(0xED, 0xFA, 0x45, 0x0B, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xB0, 0x54, 0xAF);*/

    DISPLAY_CMD(ST7701_CND2BKxSEL, 0x77, 0x01, 0x00, 0x00, 0x13);
    DISPLAY_CMD(0xEF, 0x08);
